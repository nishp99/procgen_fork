...
class parallelEnv(VecEnv):
    ...
    def reset_some(self, indices):
        for i in indices:
            self.remotes[i].send(('reset', None))
        #return np.stack([remote.recv() for remote in self.remotes])

    def start_some(self, indices):
        for i in indices:
            self.remotes[i].send(('step', 1))
            for _ in range(5):
                self.remotes[i].send(('step', np.random.choice([4,5])))
                self.remotes[i].send(('step', 0))
        self.waiting = True
