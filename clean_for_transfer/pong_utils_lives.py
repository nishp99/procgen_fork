...
def collect_trajectories(envs, policy, R, k, tmax=200, nrand=5):
	...
	for t in range(tmax):
		...
		reward = re1 + re2
		mask = np.where(reward < 0, 1, 0)
		time_lost = np.where(mask == 1, t, tmax)
		max_time = np.minimum(max_time, time_lost)
		lives -= mask
	...
	rewards_mask = np.where(lives > 0, 1, 0)
